!(function () {
  ...
  let o = {
    ...
    c = (e) => [o.prefix, e, o.suffix].filter((e) => e && e.length > 0).join("-"),
    l = (e) => {
  ...
  async function p(e, t) {
    let s = null;
    if ((e.url && (s = new URL(e.url).origin), s !== self.location.origin)) throw new d("cross-origin-copy-response", { origin: s });
    let i = e.clone(),
  ...
  async function w(e, t, a, s) {
    ...
    for (let r of await e.keys(t, n)) if (i === g(r.url, a)) return e.match(r, s);
  }
  function y(e) {
    e.then(() => {});
  }
  ...
  async function x(e) {
    ...
    let a = await self.clients.matchAll({ type: "window" }),
      ...
      i = performance.now();
    for (; performance.now() - i < 2e3 && !(t = (a = await self.clients.matchAll({ type: "window" })).find((t) => (e ? t.id === e : !s.has(t.id)))); ) await v(100);
    return t;
  }
  function R(e, t) {
    let a = t();
      ...
      "function" == typeof (t = e)
        ...
        : (t instanceof IDBTransaction &&
              (function (e) {
                ...
                let t = new Promise((t, a) => {
                  let s = () => {
                      e.removeEventListener("complete", i), e.removeEventListener("error", n), e.removeEventListener("abort", n);
                    },
                    i = () => {
                      t(), s();
                    },
            ...
            E(t, s || (s = [IDBDatabase, IDBObjectStore, IDBIndex, IDBCursor, IDBTransaction])))
          ? new Proxy(t, D)
          : t;
    return a !== e && (q.set(e, a), C.set(a, e)), a;
  }
  let k = (e) => C.get(e);
  function P(e, t, { blocked: a, upgrade: s, blocking: i, terminated: n } = {}) {
    ...
    return (
      ...
      a && r.addEventListener("blocked", (e) => a(e.oldVersion, e.newVersion, e)),
      o
        .then((e) => {
  ...
  function I(e, t) {
    ...
    let a = t.replace(/FromIndex$/, ""),
      s = t !== a,
      i = A.includes(a);
    if (!(a in (s ? IDBIndex : IDBObjectStore).prototype) || !(i || T.includes(a))) return;
    let n = async function (e, ...t) {
      let n = this.transaction(e, i ? "readwrite" : "readonly"),
        r = n.store;
      return s && (r = r.index(t.shift())), (await Promise.all([r[a](...t), i && n.done]))[0];
    };
    return L.set(t, n), n;
  }
  D = { ...(e = D), get: (t, a, s) => I(t, a) || e.get(t, a, s), has: (t, a) => !!I(t, a) || e.has(t, a) };
  ...
  async function* O(...e) {
    ...
    let a = new Proxy(t, B);
    for (M.set(a, t), C.set(a, k(t)); t; ) yield a, (t = await (W.get(a) || t.continue())), W.delete(a);
  }
  function K(e, t) {
    return (t === Symbol.asyncIterator && E(e, [IDBIndex, IDBObjectStore, IDBCursor])) || ("iterate" === t && E(e, [IDBIndex, IDBObjectStore]));
  }
  D = { ...(t = D), get: (e, a, s) => (K(e, a) ? O : t.get(e, a, s)), has: (e, a) => K(e, a) || t.has(e, a) };
  let j = "requests",
    $ = "queueName";
  class H {
    _db = null;
    ...
    async getDb() {
      return this._db || (this._db = await P("serwist-background-sync", 3, { upgrade: this._upgradeDb })), this._db;
    }
    _upgradeDb(e, t) {
      t > 0 && t < 3 && e.objectStoreNames.contains(j) && e.deleteObjectStore(j), e.createObjectStore(j, { autoIncrement: !0, keyPath: "id" }).createIndex($, $, { unique: !1 });
    }
  }
  class G {
    _queueName;
  ...
  let V = ["method", "referrer", "referrerPolicy", "mode", "credentials", "cache", "redirect", "integrity", "keepalive"];
  class Q {
    _requestData;
  ...
  let z = "serwist-background-sync",
    J = new Set(),
    X = (e) => {
      let t = { request: new Q(e.requestData).toRequest(), timestamp: e.timestamp };
  ...
  class Y {
    ...
    async getAll() {
      ...
      for (let s of e) {
        ...
        t - s.timestamp > e ? await this._queueStore.deleteEntry(s.id) : a.push(X(s));
      }
      return a;
    }
    async size() {
    ...
    async _addRequest({ request: e, metadata: t, timestamp: a = Date.now() }, s) {
      let i = { requestData: (await Q.fromRequest(e.clone())).toObject(), timestamp: a };
    ...
    async replayRequests() {
      ...
      for (; (e = await this.shiftRequest()); )
        try {
          ...
          throw (await this.unshiftRequest(e), new d("queue-replay-failed", { name: this._name }));
        }
    }
    async registerSync() {
  ...
  class ei {
    ...
    handleRequest({ request: e, event: t }) {
      ...
      let i = s.origin === location.origin,
        { params: n, route: r } = this.findMatchingRoute({ event: t, request: e, sameOrigin: i, url: s }),
        o = r?.handler,
        c = e.method;
    ...
    unregisterRoute(e) {
      if (!this._routes.has(e.method)) throw new d("unregister-route-but-not-found-with-method", { method: e.method });
  ...
  let en = () => (n || ((n = new ei()).addFetchListener(), n.addCacheListener()), n);
  function er(e, t, a) {
    let s;
    if ("string" == typeof e) {
    ...
    else throw new d("unsupported-route-type", { moduleName: "@serwist/routing", funcName: "registerRoute", paramName: "capture" });
  ...
  class ec {
    ...
    async fetch(e) {
      ...
      try {
        for (let e of this.iterateCallbacks("requestWillFetch")) a = await e({ request: a.clone(), event: t });
      } catch (e) {
        if (e instanceof Error) throw new d("plugin-error-request-will-fetch", { thrownErrorMessage: e.message });
      }
      let i = a.clone();
      try {
        let e;
        for (let s of ((e = await fetch(a, "navigate" === a.mode ? void 0 : this._strategy.fetchOptions)), this.iterateCallbacks("fetchDidSucceed")))
          e = await s({ event: t, request: i, response: e });
        return e;
      } catch (e) {
        throw (s && (await this.runCallbacks("fetchDidFail", { error: e, event: t, originalRequest: s.clone(), request: i.clone() })), e);
      }
    }
    ...
    async cacheMatch(e) {
      ...
      for (let e of ((t = await caches.match(n, r)), this.iterateCallbacks("cachedResponseWillBeUsed")))
        t = (await e({ cacheName: s, matchOptions: i, cachedResponse: t, request: n, event: this.event })) || void 0;
      return t;
    }
    async cachePut(e, t) {
      let a = eo(e);
      ...
      try {
        ...
        if (e instanceof Error) throw ("QuotaExceededError" === e.name && (await _()), e);
      }
      for (let e of this.iterateCallbacks("cacheDidUpdate")) await e({ cacheName: n, oldResponse: l, newResponse: i.clone(), request: s, event: this.event });
      return !0;
    }
    async getCacheKey(e, t) {
      ...
      if (!this._cacheKeys[a]) {
        let s = e;
        for (let e of this.iterateCallbacks("cacheKeyWillBeUsed")) s = eo(await e({ mode: t, request: s, event: this.event, params: this.params }));
        this._cacheKeys[a] = s;
      }
    ...
    *iterateCallbacks(e) {
    ...
    async _ensureResponseSafeToCache(e) {
      let t = e,
        a = !1;
      for (let e of this.iterateCallbacks("cacheWillUpdate")) if (((t = (await e({ request: this.request, response: t, event: this.event })) || void 0), (a = !0), !t)) break;
  ...
  class el {
    ...
    handleAll(e) {
      ...
      let t = e.event,
        ...
        s = new ec(this, { event: t, request: a, params: "params" in e ? e.params : void 0 }),
    ...
    async _getResponse(e, t, a) {
      ...
      try {
        if (((s = await this._handle(t, e)), void 0 === s || "error" === s.type)) throw new d("no-response", { url: t.url });
      } catch (i) {
        if (i instanceof Error) {
          for (let n of e.iterateCallbacks("handlerDidError")) if (void 0 !== (s = await n({ error: i, event: a, request: t }))) break;
        }
        if (!s) throw i;
      }
      for (let i of e.iterateCallbacks("handlerWillRespond")) s = await i({ event: a, request: t, response: s });
      return s;
    }
    async _awaitComplete(e, t, a, s) {
      ...
      try {
        await t.runCallbacks("handlerDidRespond", { event: s, request: a, response: i }), await t.doneWaiting();
      } catch (e) {
        e instanceof Error && (n = e);
      }
      if ((await t.runCallbacks("handlerDidComplete", { event: s, request: a, response: i, error: n }), t.destroy(), n)) throw n;
    }
  }
  class eh extends el {
    async _handle(e, t) {
  ...
  let ed = { cacheWillUpdate: async ({ response: e }) => (200 === e.status || 0 === e.status ? e : null) };
  class ef extends el {
    _networkTimeoutSeconds;
    constructor(e = {}) {
      super(e), this.plugins.some((e) => "cacheWillUpdate" in e) || this.plugins.unshift(ed), (this._networkTimeoutSeconds = e.networkTimeoutSeconds || 0);
    }
    async _handle(e, t) {
      let a;
      let s = [],
      ...
      if (this._networkTimeoutSeconds) {
        let { id: n, promise: r } = this._getTimeoutPromise({ request: e, logs: s, handler: t });
        (a = n), i.push(r);
      }
  ...
  class ep extends el {
    ...
    async _handle(e, t) {
      ...
      try {
        ...
        if (!(a = await Promise.race(s))) throw Error(`Timed out the network response after ${this._networkTimeoutSeconds} seconds.`);
      } catch (e) {
  ...
  class em extends el {
    constructor(e = {}) {
      super(e), this.plugins.some((e) => "cacheWillUpdate" in e) || this.plugins.unshift(ed);
    }
    async _handle(e, t) {
      let a;
  ...
  let eg = "www.google-analytics.com",
      ...
      async ({ queue: t }) => {
        ...
        for (; (a = await t.shiftRequest()); ) {
          let { request: s, timestamp: i } = a,
            n = new URL(s.url);
          try {
            let t = "POST" === s.method ? new URLSearchParams(await s.clone().text()) : n.searchParams,
            ...
            "function" == typeof e.hitFilter && e.hitFilter.call(null, t),
              await fetch(new Request(n.origin + n.pathname, { body: t.toString(), method: "POST", mode: "cors", credentials: "omit", headers: { "Content-Type": "text/plain" } }));
          } catch (e) {
    ...
    ev = (e) => new et(({ url: e }) => e.hostname === eg && "/analytics.js" === e.pathname, new ef({ cacheName: e }), "GET"),
    ex = (e) => new et(({ url: e }) => e.hostname === ew && "/gtag/js" === e.pathname, new ef({ cacheName: e }), "GET"),
    eR = (e) => new et(({ url: e }) => e.hostname === ew && "/gtm.js" === e.pathname, new ef({ cacheName: e }), "GET"),
    eE = (e = {}) => {
      let t = h.getGoogleAnalyticsName(e.cacheName),
        a = new Z("serwist-google-analytics", { maxRetentionTime: 2880, onSync: e_(e) }),
  ...
  class eS extends el {
    ...
    static copyRedirectedCacheableResponsesPlugin = { cacheWillUpdate: async ({ response: e }) => (e.redirected ? await p(e) : e) };
    constructor(e = {}) {
    ...
    async _handleFetch(e, t) {
      ...
      if (this._fallbackToNetwork) {
        let i = s.integrity,
          n = e.integrity,
          r = !n || n === i;
        (a = await t.fetch(new Request(e, { integrity: "no-cors" !== e.mode ? n || i : void 0 }))),
          i && r && "no-cors" !== e.mode && (this._useDefaultCacheabilityPluginIfNeeded(), await t.cachePut(e, a.clone()));
      } else throw new d("missing-precache-entry", { cacheName: this.cacheName, url: e.url });
      return a;
    }
    async _handleInstall(e, t) {
      this._useDefaultCacheabilityPluginIfNeeded();
      let a = await t.fetch(e);
      if (!(await t.cachePut(e, a.clone()))) throw new d("bad-precaching-response", { url: e.url, status: a.status });
      return a;
    }
    _useDefaultCacheabilityPluginIfNeeded() {
      ...
      for (let [a, s] of this.plugins.entries())
        s !== eS.copyRedirectedCacheableResponsesPlugin && (s === eS.defaultPrecacheCacheabilityPlugin && (e = a), s.cacheWillUpdate && t++);
      0 === t ? this.plugins.push(eS.defaultPrecacheCacheabilityPlugin) : t > 1 && null !== e && this.plugins.splice(e, 1);
    }
  }
  class eq {
    _precacheController;
  ...
  class eD {
    ...
    constructor({ cacheName: e, plugins: t = [], fallbackToNetwork: a = !0 } = {}) {
      (this._strategy = new eS({ cacheName: h.getPrecacheName(e), plugins: [...t, new eq({ precacheController: this })], fallbackToNetwork: a })),
    ...
    addToCacheList(e) {
      ...
      for (let a of e) {
        ...
        let { cacheKey: e, url: s } = (function (e) {
            ...
            if (!a) throw new d("add-to-cache-list-unexpected-type", { entry: e });
            if (!t) {
        ...
        if (this._urlsToCacheKeys.has(s) && this._urlsToCacheKeys.get(s) !== e)
          throw new d("add-to-cache-list-conflicting-entries", { firstEntry: this._urlsToCacheKeys.get(s), secondEntry: e });
        if ("string" != typeof a && a.integrity) {
          if (this._cacheKeysToIntegrities.has(e) && this._cacheKeysToIntegrities.get(e) !== a.integrity) throw new d("add-to-cache-list-conflicting-integrities", { url: s });
...
This is generally NOT safe. Learn more at https://bit.ly/wb-precache`);
      ...
      return R(e, async () => {
        ...
        for (let [a, s] of (this.strategy.plugins.push(t), this._urlsToCacheKeys)) {
          let t = this._cacheKeysToIntegrities.get(s),
            ...
            n = new Request(a, { integrity: t, cache: i, credentials: "same-origin" });
          await Promise.all(this.strategy.handleAll({ params: { cacheKey: s }, request: n, event: e }));
        }
    ...
    createHandlerBoundToURL(e) {
      ...
      return (a) => ((a.request = new Request(e)), (a.params = { cacheKey: t, ...a.params }), this.strategy.handle(a));
    }
  }
  let eN = () => (r || (r = new eD()), r);
  class ek {
  ...
  class eP extends et {
    constructor(e, t) {
      super(({ request: a }) => {
        ...
        for (let i of (function* (e, { ignoreURLParametersMatching: t = [/^utm_/, /^fbclid$/], directoryIndex: a = "index.html", cleanURLs: s = !0, urlManipulation: i } = {}) {
  ...
  let eT = "-precache-",
    eA = async (e, t = eT) => {
      let a = (await self.caches.keys()).filter((a) => a.includes(t) && a.includes(self.registration.scope) && a !== e);
  ...
  let eI = (e, t, a) =>
    ...
    eF = "undefined" != typeof navigator && /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
  function eW(e) {
  ...
  class eM {
    ...
    _generatePayload;
    _notifyAllClients;
    constructor({ generatePayload: e, headersToCheck: t, notifyAllClients: a } = {}) {
      (this._headersToCheck = t || eU), (this._generatePayload = e || eW), (this._notifyAllClients = a ?? !0);
    }
    async notifyIfUpdated(e) {
      if (e.oldResponse && !eI(e.oldResponse, e.newResponse, this._headersToCheck)) {
        let t = { type: "CACHE_UPDATED", meta: "serwist-broadcast-update", payload: this._generatePayload(e) };
        if ("navigate" === e.request.mode) {
          let t;
          e.event instanceof FetchEvent && (t = e.event.resultingClientId), (!(await x(t)) || eF) && (await v(3500));
        }
  ...
  class eH {
      ...
      this._upgradeDb(e),
        this._cacheName &&
          (function (e, { blocked: t } = {}) {
            ...
            t && a.addEventListener("blocked", (e) => t(e.oldVersion, e)), N(a).then(() => void 0);
          })(this._cacheName);
    }
    async setTimestamp(e, t) {
      let a = { url: (e = e$(e)), timestamp: t, cacheName: this._cacheName, id: this._getId(e) },
        s = (await this.getDb()).transaction(ej, "readwrite", { durability: "relaxed" });
    ...
    async expireEntries(e, t) {
      let a = await this.getDb(),
        s = await a.transaction(ej).store.index("timestamp").openCursor(null, "prev"),
        i = [],
        n = 0;
      for (; s; ) {
        let a = s.value;
        a.cacheName === this._cacheName && ((e && a.timestamp < e) || (t && n >= t) ? i.push(s.value) : n++), (s = await s.continue());
      }
      let r = [];
    ...
    async getDb() {
      return this._db || (this._db = await P("serwist-expiration", 1, { upgrade: this._upgradeDbAndDeleteOldDbs.bind(this) })), this._db;
    }
  }
  class eG {
    _isRunning = !1;
    ...
    _timestampModel;
    constructor(e, t = {}) {
      (this._maxEntries = t.maxEntries), (this._maxAgeSeconds = t.maxAgeSeconds), (this._matchOptions = t.matchOptions), (this._cacheName = e), (this._timestampModel = new eH(e));
    }
    async expireEntries() {
      if (this._isRunning) {
  ...
  class eV {
    ...
    _cacheExpirations;
    constructor(e = {}) {
      (this._config = e), (this._maxAgeSeconds = e.maxAgeSeconds), (this._cacheExpirations = new Map()), e.purgeOnQuotaError && f.add(() => this.deleteCacheAndMetadata());
    }
    ...
    cachedResponseWillBeUsed = async ({ event: e, request: t, cacheName: a, cachedResponse: s }) => {
      if (!s) return null;
  ...
  async function eQ(e, t) {
    try {
      ...
      let s = (function (e) {
          ...
          if (!t.startsWith("bytes=")) throw new d("unit-must-be-bytes", { normalizedRangeHeader: t });
          if (t.includes(",")) throw new d("single-range-only", { normalizedRangeHeader: t });
          let a = /(\d*)-(\d*)/.exec(t);
          if (!a || !(a[1] || a[2])) throw new d("invalid-range-values", { normalizedRangeHeader: t });
          return { start: "" === a[1] ? void 0 : Number(a[1]), end: "" === a[2] ? void 0 : Number(a[2]) };
        })(a),
        i = await t.blob(),
        n = (function (e, t, a) {
          let s, i;
          let n = e.size;
          if ((a && a > n) || (t && t < 0)) throw new d("range-not-satisfiable", { size: n, end: a, start: t });
          return (
            void 0 !== t && void 0 !== a ? ((s = t), (i = a + 1)) : void 0 !== t && void 0 === a ? ((s = t), (i = n)) : void 0 !== a && void 0 === t && ((s = n - a), (i = n)),
            { start: s, end: i }
          );
        })(i, s.start, s.end),
        r = i.slice(n.start, n.end),
        o = r.size,
        c = new Response(r, { status: 206, statusText: "Partial Content", headers: t.headers });
      return c.headers.set("Content-Length", String(o)), c.headers.set("Content-Range", `bytes ${n.start}-${n.end - 1}/${i.size}`), c;
    } catch (e) {
      return new Response("", { status: 416, statusText: "Range Not Satisfiable" });
    }
  }
  class ez {
    cachedResponseWillBeUsed = async ({ request: e, cachedResponse: t }) => (t && e.headers.has("range") ? await eQ(e, t) : t);
  }
  let eJ = () => {
          ...
          !e.options ||
            ...
            e.options.plugins.push({
              async handlerDidError(e) {
                for (let { matcher: a, url: s, cacheMatchOptions: i = { ignoreSearch: !0 } } of t) if (a(e)) return caches.match(s, i);
    ...
    eY = ({ precacheEntries: e, precacheOptions: t, cleanupOutdatedCaches: a = !1, ...s }) => {
      if (
        (e && e.length > 0 && eL(e, t),
        a &&
        ...
        var i;
        er(new ea(((i = s.navigateFallback), eN().createHandlerBoundToURL(i)), { allowlist: s.navigateFallbackAllowlist, denylist: s.navigateFallbackDenylist }));
      }
    },
    eZ = (e) => null != e,
    e0 = { CacheFirst: eh, CacheOnly: eu, NetworkFirst: ef, NetworkOnly: ep, StaleWhileRevalidate: em },
    e1 = (...e) => {
      for (let t of e)
  ...
  (({
    ...
    i && i.length > 0 && self.importScripts(...i),
      ...
      void 0 !== c && (void 0 !== d && (c = eX({ ...d, runtimeCaching: c })), e1(...c)),
      void 0 !== l && ("boolean" == typeof l ? l && eE() : eE(l)),
      u && eJ();
  })({
    precacheEntries: [
      { revision: "2b54d7db375d2b4c0e6af318090bebea", url: "/_next/static/HNNcv1E27cw-KUqYGNrBw/_buildManifest.js" },
      { revision: "b6652df95db52feb4daf4eca35380933", url: "/_next/static/HNNcv1E27cw-KUqYGNrBw/_ssgManifest.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/0e9f9a68-8ec06a8511c123e0.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/1735-f1a3d664cf562886.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/2356-0ea08df9072cea8e.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/2832-faa9698897fb205a.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/29d61979-34faa766e319423b.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/3932-ef9e6cdd5cdb0ae8.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/4539-80bcd4db41d812b3.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/5250-c61320ef7ed053b0.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/529-b2bfb407b136dcd6.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/5516-d0f2c101a282370f.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/5688-8448ae4a0ee3f972.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/596-684b8b46031a7a9e.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/5998-144dc11005294a26.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/5c4a068a-0770db2084ada18a.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/6208-193f77e3238bc1ea.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/6298-783c931dd227996b.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/6446-7e1403244f967544.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/6476-1968a63b0295921d.js" },
      { revision: "af849eaf1c2be18e", url: "/_next/static/chunks/6a487de7.af849eaf1c2be18e.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/7066-3b5b9101beacf229.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/7188-5794ab4b20bfbc5f.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/7216-8557e73b120c4e8d.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/7372-3eca580bd8f67572.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/8775-4662aa3d3e66467a.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/8986-589b18ad7466311f.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/9279-eca6b9a18dde2976.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/9456-7c0c5685ac4fc69e.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/(editor)/portal/posts/%5Bid%5D/layout-88d9a0150b046d38.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/(editor)/portal/posts/%5Bid%5D/page-25a750cd99d15f94.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/(home)/about/page-4a031e39031ad7c2.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/(home)/layout-ce1022a19bc47a0b.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/(home)/loading-fd724ceb2cafb889.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/(home)/page-1af0eedb348fb7ce.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/_not-found-455f2dd41c630615.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/error-cc4437c384d29651.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/layout-f193efeea9621d48.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/portal/courses/loading-c24f373b8fcc9d6e.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/portal/courses/page-9cd3697c97021ec2.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/portal/layout-2e744e18320acc4f.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/portal/page-832d415e901ea8c3.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/portal/posts/loading-5f8abd35bff356f1.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/portal/posts/page-ef3b3d91fc182ba4.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/portal/products/loading-32da42d9b8ecba6a.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/app/portal/products/page-d78bf0b5e14bfdc2.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/c36591c5-ed5ccc14dbf29ca4.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/d6a3c14e-8855dfbba317398b.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/fd9d1056-d07ef1d5fd4a71fc.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/framework-08aa667e5202eed8.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/main-2791e5e8ed3f6142.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/main-app-13d8ea9919f9ad37.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/pages/_app-57bdff7978360b1c.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/pages/_error-29037c284dd0eec6.js" },
      { revision: "837c0df77fd5009c9e46d446188ecfd0", url: "/_next/static/chunks/polyfills-c67a75d1b6f99dc8.js" },
      { revision: "HNNcv1E27cw-KUqYGNrBw", url: "/_next/static/chunks/webpack-d267b00f985dbafd.js" },
      { revision: "2a3a51db3021cd98", url: "/_next/static/css/2a3a51db3021cd98.css" },
      { revision: "36a18750b35250ea", url: "/_next/static/css/36a18750b35250ea.css" },
      { revision: "5711013747473e62", url: "/_next/static/css/5711013747473e62.css" },
      { revision: "f1b44860c66554b91f3b1c81556f73ca", url: "/_next/static/media/05a31a2ca4975f99-s.woff2" },
      { revision: "9b2795fb691d8f8a83312a4436f5a453", url: "/_next/static/media/122c360d7fe6d395-s.p.woff2" },
      { revision: "c4eb7f37bc4206c901ab08601f21f0f2", url: "/_next/static/media/513657b02c5c193f-s.woff2" },
      { revision: "bb9d99fb9bbc695be80777ca2c1c2bee", url: "/_next/static/media/51ed15f9841b9f9d-s.woff2" },
      { revision: "183db31d6365283bef4914042be9dfab", url: "/_next/static/media/90475aac776488b6-s.p.woff2" },
      ...
      { revision: "74c3556b9dad12fb76f84af53ba69410", url: "/_next/static/media/c9a5bc6a7c948fb0-s.p.woff2" },
      ...
      { revision: "0e89df9522084290e01e4127495fae99", url: "/_next/static/media/ec159349637c90ad-s.woff2" },
      { revision: "71f3fcaf22131c3368d9ec28ef839831", url: "/_next/static/media/fd4db3eb5472fc27-s.woff2" },
    ...
    runtimeCaching: [
      ...
      { urlPattern: /\.(?:css|less)$/i, handler: "StaleWhileRevalidate", options: { cacheName: "static-style-assets", expiration: { maxEntries: 32, maxAgeSeconds: 86400 } } },
      { urlPattern: /\/_next\/data\/.+\/.+\.json$/i, handler: "StaleWhileRevalidate", options: { cacheName: "next-data", expiration: { maxEntries: 32, maxAgeSeconds: 86400 } } },
      ...
      },
      {
        urlPattern: ({ request: e, url: { pathname: t }, sameOrigin: a }) =>
          "1" === e.headers.get("RSC") && "1" === e.headers.get("Next-Router-Prefetch") && a && !t.startsWith("/api/"),
        handler: "NetworkFirst",
