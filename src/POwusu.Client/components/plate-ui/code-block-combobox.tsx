  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" role="combobox" aria-expanded={open} className="h-5 justify-between px-1 text-xs" size="xs">
          {state.value ? languages.find((language) => language.value === state.value)?.label : "Plain Text"}
          <Icons.chevronsUpDown className="size-4 ml-2 shrink-0 opacity-50" />
        </Button>
      </PopoverTrigger>
        ...
        <Command>
          ...
          <CommandList>
            {languages.map((language) => (
              ...
              >
                <Icons.check className={cn("size-4 mr-2", state.value === language.value ? "opacity-100" : "opacity-0")} />
                {language.label}
              </CommandItem>
            ))}
          </CommandList>
