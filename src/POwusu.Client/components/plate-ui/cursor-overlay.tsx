...
export function Cursor({ data, selectionRects, caretPosition, disableCaret, disableSelection, classNames }: CursorProps<CursorData>) {
  if (!data) {
    return null;
  }
  ...
  return (
    <>
      ...
      {!disableCaret && caretPosition && <div className={cn("pointer-events-none absolute z-10 w-0.5", classNames?.caret)} style={{ ...caretPosition, ...style }} />}
    </>
  );
}
