...
export type EmojiPickerContentProps = Pick<
  UseEmojiPickerType,
  "i18n" | "onMouseOver" | "onSelectEmoji" | "emojiLibrary" | "isSearching" | "searchResult" | "visibleCategories" | "refs" | "settings"
>;
...
const Button = memo(({ index, emoji, onSelect, onMouseOver }: EmojiButtonProps) => {
  return (
    <button
      ...
      className="size-[36px] group relative flex cursor-pointer items-center justify-center border-none bg-transparent text-2xl leading-none"
    >
      <div aria-hidden="true" className="absolute inset-0 rounded-full bg-[rgba(0,0,0,0.05)] opacity-0 group-hover:opacity-100" />
...
const RowOfButtons = memo(({ row, emojiLibrary, onSelectEmoji, onMouseOver }: RowOfButtonsProps) => (
  <div key={row.id} data-index={row.id} className="flex">
    {row.elements.map((emojiId, index) => (
      <Button key={emojiId} index={index} emoji={emojiLibrary.getEmoji(emojiId)} onSelect={onSelectEmoji} onMouseOver={onMouseOver} />
    ))}
  </div>
));
      ...
      .map(({ id: categoryId }) => {
        ...
        return (
          <div key={categoryId} data-id={categoryId} ref={section.root} style={{ width: getRowWidth }}>
            <div className="sticky -top-px z-[1] bg-background/90 p-1 backdrop-blur-[4px]">{i18n.categories[categoryId]}</div>
            <div className="relative flex flex-wrap" style={{ height: section.getRows().length * buttonSize.value }}>
              {isCategoryVisible(categoryId) &&
                section
                  .getRows()
                  .map((row: GridRow, index) => <RowOfButtons key={index} emojiLibrary={emojiLibrary} row={row} onSelectEmoji={onSelectEmoji} onMouseOver={onMouseOver} />)}
            </div>
          </div>
        );
      });
  ...
  const SearchList = useCallback(() => {
    return (
      <div data-id="search" style={{ width: getRowWidth }}>
        <div className="sticky -top-px z-[1] bg-background/90 p-1 backdrop-blur-[4px]">{i18n.searchResult}</div>
        <div className="relative flex flex-wrap">
          {searchResult.map((emoji: Emoji, index: number) => (
            <Button key={emoji.id} index={index} emoji={emojiLibrary.getEmoji(emoji.id)} onSelect={onSelectEmoji} onMouseOver={onMouseOver} />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div
      className={cn(
        ...
        "[&::-webkit-scrollbar-button]:size-0 [&::-webkit-scrollbar-button]:hidden",
