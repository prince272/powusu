...
export type EmojiPickerNavigationProps = Pick<UseEmojiPickerType, "i18n" | "emojiLibrary" | "icons" | "focusedCategory"> & {
...
export function EmojiPickerNavigation({ i18n, icons, emojiLibrary, focusedCategory, onClick }: EmojiPickerNavigationProps) {
  ...
  return (
    <nav id="emoji-nav" className="mb-2.5 border-0 border-b border-solid border-b-default-200 p-3">
      <div className="relative flex">
        {emojiLibrary
          ...
          .map(({ id }) => (
            <button
              ...
              type="button"
              className={cn(
                "flex grow cursor-pointer items-center justify-center border-none bg-transparent fill-current text-sm text-default-500 hover:text-default-800",
                id === focusedCategory && "pointer-events-none fill-current text-primary-600"
              )}
              onClick={() => onClick(id)}
            >
        ...
        <div
          className="absolute -bottom-3 left-0 h-[3px] w-full rounded-t bg-primary-600 opacity-100 transition-transform duration-200"
          style={{
